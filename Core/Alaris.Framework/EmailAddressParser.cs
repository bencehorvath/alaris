...
namespace Alaris.Framework
{
    ...
    {
        private static readonly Regex AddressRegex = new Regex("^(.+?)\\@(.+?)$", RegexOptions.Singleline | RegexOptions.RightToLeft);
        private static readonly Regex BracketRegex = new Regex("^\\<*|\\>*$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
        private static readonly Regex TrimRegex = new Regex("^\\s*|\\s*$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
        private static readonly Regex TLDomainRegex = new Regex("^((([a-z0-9-]+)\\.)+)[a-z]{2,6}$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
        private static readonly Regex AnyDomainRegex = new Regex("^((([a-z0-9-]+)\\.)+)$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
        private static readonly Regex DomainExtensionRegex;
        ...
        private bool DomainExtensionValid()
        {
            ...
            return DomainExtensionRegex.IsMatch(_domain);
        }
    }
}
