...
namespace Alaris.Core.Extensions
{
    public static class CollectionExtensions
    {
        #region Dictionary Extensions

        public static List<TV> GetOrCreate<TK, TV>(this IDictionary<TK, List<TV>> map, TK key)
        {
            List<TV> list;
            if (!map.TryGetValue(key, out list))
            {
                map.Add(key, list = new List<TV>());
            }
            return list;
        }

        public static HashSet<TV> GetOrCreate<TK, TV>(this IDictionary<TK, HashSet<TV>> map, TK key)
        {
            HashSet<TV> dict;
            if (!map.TryGetValue(key, out dict))
            {
                map.Add(key, dict = new HashSet<TV>());
            }
            return dict;
        }

        public static IDictionary<TK2, TV> GetOrCreate<TK, TK2, TV>(this IDictionary<TK, IDictionary<TK2, TV>> map, TK key)
        {
            IDictionary<TK2, TV> dict;
            if (!map.TryGetValue(key, out dict))
            {
                map.Add(key, dict = new Dictionary<TK2, TV>());
            }
            return dict;
        }

        public static Dictionary<TK2, TV> GetOrCreate<TK, TK2, TV>(this IDictionary<TK, Dictionary<TK2, TV>> map, TK key)
        {
            Dictionary<TK2, TV> dict;
            if (!map.TryGetValue(key, out dict))
            {
                map.Add(key, dict = new Dictionary<TK2, TV>());
            }
        ...
        public static TV GetValue<TK, TV>(this IDictionary<TK, TV> map, TK key)
        {
            TV val;
                                                                   ...
                                                                   Func<TInput, TOutput> transformer)
        {
            ...
            output.AddRange(enumerable.Select(transformer));
        ...
        public static Type GetActualType(this MemberInfo member)
        {
            ...
            if (isArr)
            {
                ...
                if (memberType == null)
                {
                    throw new Exception(string.Format("Unable to get Type of Array {0} ({1}).", rawType,
