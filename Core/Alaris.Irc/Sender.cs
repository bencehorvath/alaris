...
namespace Alaris.Irc
{
    ...
    public class Sender : CommandBuilder
    {
        ...
        private static bool IsEmpty(string aString)
        {
        ...
        /// <returns></returns>
        private static string Truncate(string parameter, int commandLength)
        {
            var max = MaxCommandSize - commandLength;

            return parameter.Length > max ? parameter.Substring(0, max) : parameter;
        }

        private static bool TooLong(StringBuilder buffer)
        {
            //2 for CR LF
            return (buffer.Length + 2) > MaxCommandSize;
        }
        ...
        public void PublicNotice(string channel, string message)
        {
            lock (this)
            {
                ...
                if (Rfc2812Util.IsValidChannelName(channel))
                {
                    ...
                    int max = MaxCommandSize - 11 - channel.Length;
                    if (message.Length > max)
                    {
                        IEnumerable<string> parts = BreakUpMessage(message, max);
        ...
        public void PrivateNotice(string nick, string message)
        {
            lock (this)
            {
                ...
                {
                    // 11 is NOTICE + 2 x Spaces + : + CR + LF
                    int max = MaxCommandSize - 11 - nick.Length;
                    if (message.Length > max)
                    {
                        IEnumerable<string> parts = BreakUpMessage(message, max);
                        foreach (var part in parts)
        ...
        /// <summary>
        ///   Changes this client's mode. To change another nick's mode
        ///   use <see cref = "ChangeChannelMode" />.
        /// </summary>
        /// <remarks>
        ...
        public void RequestChannelList(string channel, ChannelMode mode)
        {
            lock (this)
            {
                ...
                {
                    ClearBuffer();
                    throw new ArgumentException(Enum.GetName(typeof (ChannelMode), mode) +
                                                " is not a valid channel mode for this request.");
                }
                Buffer.Append("MODE");
                Buffer.Append(Space);
                Buffer.Append(channel);
        ...
        public void Raw(string message)
        {
            lock (this)
            {
                ...
                if (message.Length > MaxCommandSize)
                {
                    message = message.Substring(0, MaxCommandSize);
                }
                Buffer.Append(message);
                Connection.SendCommand(Buffer);
            }
